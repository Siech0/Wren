...
# hack: if a new cmake (which uses CMAKE_LINKER) runs on an old build tree (where link was hardcoded) and where
# CMAKE_LINKER isn't in the cache and still cmake didn't fail in CMakeFindBinUtils.cmake (because it isn't rerun)
...
elseif(WINDOWS_PHONE OR WINDOWS_STORE)
  ...
  elseif(
    _MSVC_C_ARCHITECTURE_FAMILY STREQUAL "ARM"
    OR _MSVC_CXX_ARCHITECTURE_FAMILY STREQUAL "ARM"
    OR _MSVC_C_ARCHITECTURE_FAMILY STREQUAL "ARM64"
    OR _MSVC_CXX_ARCHITECTURE_FAMILY STREQUAL "ARM64"
  )
  ...
  elseif(MSVC_VERSION GREATER 1310)
    if(CMAKE_VS_PLATFORM_TOOLSET MATCHES "v[0-9]+_clang_.*")
      # Clang/C2 in MSVC14 Update 1 seems to not support -fsantinize (yet?) set(_RTC1 "-fsantinize=memory,safe-stack")
...
# add /debug and /INCREMENTAL:YES to DEBUG and RELWITHDEBINFO also add pdbtype on versions that support it
...
macro(__windows_compiler_msvc_clang lang)
  if(NOT MSVC_VERSION LESS 1400)
    ...
    set(_CMAKE_VS_LINK_DLL
        "<CMAKE_COMMAND> -E vs_link_dll --intdir=<OBJECT_DIR> --rc=<CMAKE_RC_COMPILER> --mt=<CMAKE_MT> --manifests <MANIFESTS> -- "
    )
    set(_CMAKE_VS_LINK_EXE
        "<CMAKE_COMMAND> -E vs_link_exe --intdir=<OBJECT_DIR> --rc=<CMAKE_RC_COMPILER> --mt=<CMAKE_MT> --manifests <MANIFESTS> -- "
    )
  endif()
  ...
  if(CMAKE_${lang}_COMPILER_ID STREQUAL "Clang")
    ...
    # macOS paths usually start with /Users/*. Unfortunately, clang-cl interprets paths starting with /U as macro
    # undefines, so we need to put a -- before the input file path to force it to be treated as a path.
    string(REPLACE "-c <SOURCE>" "-c -- <SOURCE>" CMAKE_${lang}_COMPILE_OBJECT "${CMAKE_${lang}_COMPILE_OBJECT}")
    string(REPLACE "-c <SOURCE>" "-c -- <SOURCE>" CMAKE_${lang}_CREATE_PREPROCESSED_SOURCE
                   "${CMAKE_${lang}_CREATE_PREPROCESSED_SOURCE}"
    )
    string(REPLACE "-c <SOURCE>" "-c -- <SOURCE>" CMAKE_${lang}_CREATE_ASSEMBLY_SOURCE
  ...
  if("x${CMAKE_${lang}_COMPILER_ID}" STREQUAL "xMSVC")
    ...
    string(REPLACE "<LINK_FLAGS> " "/LTCG <LINK_FLAGS> " CMAKE_${lang}_CREATE_STATIC_LIBRARY_IPO
                   "${CMAKE_${lang}_CREATE_STATIC_LIBRARY}"
    )
  elseif("x${CMAKE_${lang}_COMPILER_ID}" STREQUAL "xClang" OR "x${CMAKE_${lang}_COMPILER_ID}" STREQUAL "xFlang")
    set(_CMAKE_${lang}_IPO_SUPPORTED_BY_CMAKE YES)
    set(_CMAKE_${lang}_IPO_MAY_BE_SUPPORTED_BY_COMPILER YES)

    # '-flto=thin' available since Clang 3.9 and Xcode 8 * http://clang.llvm.org/docs/ThinLTO.html#clang-llvm *
  ...
  if("x${lang}" STREQUAL "xC" OR "x${lang}" STREQUAL "xCXX")
    ...
    if(CMAKE_VS_PLATFORM_TOOLSET MATCHES "v[0-9]+_clang_.*")
      # note: MSVC 14 2015 Update 1 sets -fno-ms-compatibility by default, but this does not allow one to compile many
      # projects that include MS's own headers. CMake itself is affected project too.
      string(
        APPEND
        CMAKE_${lang}_FLAGS_INIT
        " ${_PLATFORM_DEFINES}${_PLATFORM_DEFINES_${lang}} -fms-extensions -fms-compatibility -D_WINDOWS${_Wall}${_FLAGS_${lang}}"
      )
      ...
      string(APPEND CMAKE_${lang}_FLAGS_MINSIZEREL_INIT "${_MD} -DNDEBUG") # TODO: Add '-Os' once VS generator maps it
                                                                           # properly for Clang
    else()
      string(APPEND CMAKE_${lang}_FLAGS_INIT
             " ${_PLATFORM_DEFINES}${_PLATFORM_DEFINES_${lang}} /D_WINDOWS${_W3}${_FLAGS_${lang}}"
      )
...
# __windows_compiler_msvc(C) __windows_compiler_msvc(CXX)
