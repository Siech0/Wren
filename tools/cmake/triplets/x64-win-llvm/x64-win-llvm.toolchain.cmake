...
function(get_vcpkg_triplet_variables)
  ...
  # Be careful here you don't want to pull in all variables from the triplet! Port is not defined!
  set(VCPKG_CRT_LINKAGE
      "${VCPKG_CRT_LINKAGE}"
      PARENT_SCOPE
  ) # This is also forwarded by vcpkg itself
endfunction()
...
# -Wno-implicit-function-declaration because a lot of libraries don't #include <io.h> for read/open/access and clang 16
# made that an error instead of a warning.

# Set C++ standard. set(CMAKE_CXX_STANDARD 17 CACHE STRING "") set(CMAKE_CXX_STANDARD_REQUIRED ON CACHE STRING "")
# set(CMAKE_CXX_EXTENSIONS OFF CACHE STRING "") set(std_cxx_flags "/permissive- -std:c++20 /Zc:__cplusplus")
...
string(APPEND windows_defs " /D_CRT_INTERNAL_NONSTDC_NAMES /D_CRT_DECLARE_NONSTDC_NAMES") # due to -D__STDC__=1 required
                                                                                          # for e.g. _fopen -> fopen and
                                                                                          # other not underscored
                                                                                          # functions/defines
...
# -mcrc32 for libpq -mrtm for tbb (will break qtdeclarative since it cannot run the executables in CI) -msse4.2 for
# everything which normally cl can use. (Otherwise strict sse2 only.) -maes -mpclmul mbedtls
...
set(CMAKE_AR
    "${LLVM_BIN_DIR}/llvm-lib.exe"
    CACHE STRING ""
)
# set(CMAKE_AR "${LLVM_BIN_DIR}/llvm-ar.exe" CACHE STRING "") set(CMAKE_RANLIB "${LLVM_BIN_DIR}/llvm-ranlib.exe" CACHE
# STRING "")
set(CMAKE_LINKER
    "${LLVM_BIN_DIR}/lld-link.exe"
    CACHE STRING ""
)
# set(CMAKE_LINKER "${CLANG-CL_EXECUTABLE}" CACHE STRING "") set(CMAKE_LINKER "link.exe" CACHE STRING "" FORCE)
...
if(NOT CUDA_C_COMPILER)
  # Due to nvcc error   : 'cudafe++' died with status 0xC0000409 |  clang-cl cannot currently be used to compile cu
  # files. The CUDA frontend probably has problems parsing preprocessed files from clang-cl
...
# Set compiler flags. set(CLANG_FLAGS "/clang:-fasm -fmacro-backtrace-limit=0") #/clang:-fopenmp-simd -openmp
...
# https://devblogs.microsoft.com/cppblog/asan-for-windows-x64-and-debug-build-support/ dynamic CRT case is not allowed
# to have /wholearchive!
set(sanitizer_path "")
set(sanitizer_libs "")
...
if(VCPKG_USE_SANITIZERS)
  ...
  if(VCPKG_USE_LTO)
    ...
    string(APPEND sanitizers ",address") # lld-link: error: /alternatename: conflicts:
                                         # __sanitizer_on_print=__sanitizer_on_print__def
  endif()
  ...
  if(NOT VCPKG_USE_COMPILER_FOR_LINKAGE)
    # set(ENV{LINK} "$ENV{LINK} /LIBPATH:\"${clang_ver_path}/lib/windows\"") set(sanitizer_path
    # "/LIBPATH:\\\\\"${clang_ver_path}/lib/windows\\\\\"" )
    if(VCPKG_CRT_LINKAGE STREQUAL "dynamic")
      set(sanitizer_libs_exe
          "-include:__asan_seh_interceptor clang_rt.asan_dynamic-x86_64.lib clang_rt.asan_dynamic_runtime_thunk-x86_64.lib /wholearchive:clang_rt.asan_dynamic_runtime_thunk-x86_64.lib"
      )
      ...
      set(sanitizer_libs_exe
          "${sanitizer_libs} /wholearchive:clang_rt.asan-x86_64.lib /wholearchive:clang_rt.asan_cxx-x86_64.lib"
      )
...
set(CMAKE_C_FLAGS
    "${CMAKE_CL_NOLOGO} ${windows_defs} ${arch_flags} ${VCPKG_C_FLAGS} ${CLANG_FLAGS} ${CHARSET_FLAG} ${std_c_flags} ${ignore_werror}"
    CACHE STRING ""
)
set(CMAKE_C_FLAGS_DEBUG
    "/Od /Ob0 /GS /RTC1 /FC ${VCPKG_C_FLAGS_DEBUG} ${VCPKG_CRT_FLAG}d ${VCPKG_DBG_FLAG} /D_DEBUG"
    CACHE STRING ""
)
set(CMAKE_C_FLAGS_RELEASE
    "/O2 /Oi ${CLANG_FLAGS_RELEASE} ${VCPKG_C_FLAGS_RELEASE} ${VCPKG_CRT_FLAG} ${CLANG_C_LTO_FLAGS} ${VCPKG_DBG_FLAG} /DNDEBUG"
    CACHE STRING ""
)
set(CMAKE_C_FLAGS_MINSIZEREL
    "/O1 /Oi /Ob1 /GS- ${CLANG_FLAGS_RELEASE} ${VCPKG_C_FLAGS_RELEASE} ${VCPKG_CRT_FLAG} ${CLANG_C_LTO_FLAGS} /DNDEBUG"
    CACHE STRING ""
)
set(CMAKE_C_FLAGS_RELWITHDEBINFO
    "/O2 /Oi /Ob1 /GS- ${CLANG_FLAGS_RELEASE} ${VCPKG_C_FLAGS_RELEASE} ${VCPKG_CRT_FLAG} ${CLANG_C_LTO_FLAGS} ${VCPKG_DBG_FLAG} /DNDEBUG"
    CACHE STRING ""
)

set(CMAKE_CXX_FLAGS
    "${CMAKE_CL_NOLOGO} /EHsc /GR ${windows_defs} ${arch_flags} ${VCPKG_CXX_FLAGS} ${CLANG_FLAGS} ${CHARSET_FLAG} ${std_cxx_flags} ${ignore_werror}"
    CACHE STRING ""
)
set(CMAKE_CXX_FLAGS_DEBUG
    "${CMAKE_C_FLAGS_DEBUG} /FC ${VCPKG_CXX_FLAGS_DEBUG}"
    CACHE STRING ""
)
set(CMAKE_CXX_FLAGS_RELEASE
    "${CMAKE_C_FLAGS_RELEASE} ${VCPKG_CXX_FLAGS_RELEASE} ${CLANG_CXX_LTO_FLAGS}"
    CACHE STRING ""
)
set(CMAKE_CXX_FLAGS_MINSIZEREL
    "${CMAKE_C_FLAGS_MINSIZEREL} ${VCPKG_CXX_FLAGS_RELEASE} ${CLANG_CXX_LTO_FLAGS}"
    CACHE STRING ""
)
set(CMAKE_CXX_FLAGS_RELWITHDEBINFO
    "${CMAKE_C_FLAGS_RELWITHDEBINFO} ${VCPKG_CXX_FLAGS_RELEASE} ${CLANG_CXX_LTO_FLAGS}"
    CACHE STRING ""
)
...
foreach(linker IN ITEMS "SHARED" "MODULE" "EXE")
  set(CMAKE_${linker}_LINKER_FLAGS_INIT
      "${CMAKE_CL_NOLOGO} /INCREMENTAL:NO /Brepro ${VCPKG_LINKER_FLAGS}"
      CACHE STRING ""
  )
  set(CMAKE_${linker}_LINKER_FLAGS
      "${CMAKE_CL_NOLOGO} /INCREMENTAL:NO /Brepro ${VCPKG_LINKER_FLAGS}"
      CACHE STRING ""
  )
  set(CMAKE_${linker}_LINKER_FLAGS_DEBUG
      "/DEBUG:FULL ${VCPKG_LINKER_FLAGS_DEBUG}"
      CACHE STRING ""
  )
  set(CMAKE_${linker}_LINKER_FLAGS_RELEASE
      "/DEBUG /OPT:REF /OPT:ICF ${VCPKG_LINKER_FLAGS_RELEASE} ${sanitizer_path} ${sanitizer_libs}"
      CACHE STRING ""
  )
  set(CMAKE_${linker}_LINKER_FLAGS_MINSIZEREL
      "/DEBUG /OPT:REF /OPT:ICF ${sanitizer_path} ${sanitizer_libs}"
      CACHE STRING ""
  )
  set(CMAKE_${linker}_LINKER_FLAGS_RELWITHDEBINFO
      "/DEBUG:FULL /OPT:REF /OPT:ICF ${sanitizer_path} ${sanitizer_libs}"
      CACHE STRING ""
  )
endforeach()
...
if(VCPKG_USE_SANITIZERS)
  set(CMAKE_EXE_LINKER_FLAGS_RELEASE
      "${CMAKE_EXE_LINKER_FLAGS_RELEASE} ${sanitizer_libs_exe}"
      CACHE STRING "" FORCE
  )
  set(CMAKE_EXE_LINKER_FLAGS_MINSIZEREL
      "${CMAKE_EXE_LINKER_FLAGS_MINSIZEREL} ${sanitizer_libs_exe}"
      CACHE STRING "" FORCE
  )
  set(CMAKE_EXE_LINKER_FLAGS_RELWITHDEBINFO
      "${CMAKE_EXE_LINKER_FLAGS_RELWITHDEBINFO} ${sanitizer_libs_exe}"
      CACHE STRING "" FORCE
  )
  set(CMAKE_SHARED_LINKER_FLAGS_RELEASE
      "${CMAKE_SHARED_LINKER_FLAGS_RELEASE} ${sanitizer_libs_dll}"
      CACHE STRING "" FORCE
  )
  set(CMAKE_SHARED_LINKER_FLAGS_MINSIZEREL
      "${CMAKE_SHARED_LINKER_FLAGS_MINSIZEREL} ${sanitizer_libs_dll}"
      CACHE STRING "" FORCE
  )
  set(CMAKE_SHARED_LINKER_FLAGS_RELWITHDEBINFO
      "${CMAKE_SHARED_LINKER_FLAGS_RELWITHDEBINFO} ${sanitizer_libs_dll}"
      CACHE STRING "" FORCE
  )
  set(CMAKE_MODULE_LINKER_FLAGS_RELEASE
      "${CMAKE_MODULE_LINKER_FLAGS_RELEASE} ${sanitizer_libs_dll}"
      CACHE STRING "" FORCE
  )
  set(CMAKE_MODULE_LINKER_FLAGS_MINSIZEREL
      "${CMAKE_MODULE_LINKER_FLAGS_MINSIZEREL} ${sanitizer_libs_dll}"
      CACHE STRING "" FORCE
  )
  set(CMAKE_MODULE_LINKER_FLAGS_RELWITHDEBINFO
      "${CMAKE_MODULE_LINKER_FLAGS_RELWITHDEBINFO} ${sanitizer_libs_dll}"
      CACHE STRING "" FORCE
  )
endif()
...
foreach(lang IN ITEMS C CXX)
  foreach(linker IN ITEMS "SHARED" "MODULE" "EXE")
    set(CMAKE_${lang}_${linker}_LINKER_FLAGS
        "${CMAKE_${linker}_LINKER_FLAGS}"
        CACHE STRING ""
    )
    set(CMAKE_${lang}_${linker}_LINKER_FLAGS_DEBUG
        "${CMAKE_${linker}_LINKER_FLAGS_DEBUG}"
        CACHE STRING ""
    )
    set(CMAKE_${lang}_${linker}_LINKER_FLAGS_RELEASE
        "${CMAKE_${linker}_LINKER_FLAGS_RELEASE}"
        CACHE STRING ""
    )
    set(CMAKE_${lang}_${linker}_LINKER_FLAGS_MINSIZEREL
        "${CMAKE_${linker}_LINKER_FLAGS_MINSIZEREL}"
        CACHE STRING ""
    )
    set(CMAKE_${lang}_${linker}_LINKER_FLAGS_RELWITHDEBINFO
        "${CMAKE_${linker}_LINKER_FLAGS_RELWITHDEBINFO}"
        CACHE STRING ""
    )
  endforeach()
endforeach()
